...
def parse_show(self,
	...
	if self.show_dictionary['Anime']:
		self.episode = "-".join(
				[get_padded_episode_number(e, 3) for e in self.episode])
		self.absolute_episode = "-".join(
				[get_padded_episode_number(e, 3) for e in self.absolute_episode])
	else:
		self.episode = "-".join([get_padded_episode_number(e, 2) for e in self.episode])
		self.absolute_episode = "-".join(
				[get_padded_episode_number(e, 2) for e in self.absolute_episode])
	message.method_exit(g)
	self.parsed_relative_title = \
		set_nested_dictionary_key_value_pair(self.show_dictionary['Parsed Relative Show Title'],
...
def parse_show_title_from_show_dictionary(show,
                                          g):
	message.method_launch(g)
	create_directory_if_not_present("/".join((show.path, show.show_dictionary['Parsed Season Folder'])))
