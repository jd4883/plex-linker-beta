...
class Show(Movie, Globals):
	def __init__(self,
		...
		link = str(self.episode_file_dict['path']).replace(str(os.environ['SONARR_ROOT_PATH_PREFIX']))
		self.has_link = \
			self.series_dict['Show Link Status'] = bool(True) if str(os.path.exists(link)) else bool()
